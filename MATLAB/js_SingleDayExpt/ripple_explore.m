...
for i = 1:length(nrnTets)
    tet = nrnTets(i);
    nrns = [C_allspikes{1,r}{1,e}{1,tet}{:}];
    ...
    for j = 1:length(nrns)
        ...
        yvals = zeros(size(spikeTimes)) + (min(ydisp)-max(ydisp) + ydisp(idx)-0.5*(j-1) - 2);
...
for i = 1:length(riptData.starttime)
    x_vertices = [riptData.starttime(i),riptData.endtime(i),riptData.endtime(i),riptData.starttime(i)];
    y_vertices = [min(3*min(ydisp)-max(ydisp)),min(3*min(ydisp)-max(ydisp)),max(refV),max(refV)];
    patch(x_vertices, y_vertices, [0.4940 0.1840 0.5560],'FaceAlpha', 0.4, 'EdgeColor','none')
end
title(sprintf("Rat %d, Epoch %d",r,e))
ylabel("Voltage (mV)")
xlabel("Time (s)")
