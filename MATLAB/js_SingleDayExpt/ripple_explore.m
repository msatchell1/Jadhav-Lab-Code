...
%     load(sprintf("/mnt/10TBSpinDisk/js_SingleDayExpt/JS34_direct/EEG/JS34eegref01-%s-%s.mat",eStr,riptetsStr{rt}))
% 
...
for r = 1:size(C_allriptimes,2)
    ...
    for e = 1:17
        ...
        S_swsstate = swsStates{1,e}; % struct containing SWS info for one epoch
        S_stillstate = stillStates{1,e};
        if mod(e,2) == 1 % Sleep epochs
        ...
        allRipRates(r,e) = numRips/swsStillDur;
...
% title("Ripple Rates across Epochs")
...
% title("Total Ripple Time across Epochs")
...
% title("Avg Ripple Duration across Epochs")
