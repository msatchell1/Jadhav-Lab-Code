    ...
    for tetnum = 1:size(C_allspikes{1,r}{1,1},2) % Loop through all tetrodes for rat.
        ...
        if any(~isfield(matclustTet,{'clustattrib','waves'})) % Note the strings MUST be character vectors.
            ...
            for nrn = 1:size(clusters,2)
                ...
                nrnStruct.meanFRepochs = nrnFRepochs;
                meanrate = mean(nrnFRepochs,2,'omitnan');
                nrnStruct.meanrate = meanrate;
                
                allNrnStructs{tetnum,r}{nrn} = nrnStruct;

                % % Save changes back to neuron struct
                % C_allspikes{1,r}{1,e}{1,tetnum}{1,nrn}.epochFRs = nrnFRepochs;
                % C_allspikes{1,r}{1,e}{1,tetnum}{1,nrn}.acrossEpochFR = meanrate;
                % C_allspikes{1,r}{1,e}{1,tetnum}{1,nrn}.spikeWidth = spikewidth_s;
                % C_allspikes{1,r}{1,e}{1,tetnum}{1,nrn}.nrnType = 
            ...
            % interneuron.
            % 4) Find neurons in allNrnStructs that don't exist in C_allspikes
            % and see if this is because they did not spike enough.
        end
    end
...
%% Plot the spikewidth vs meanrate data


for a = 1:size(brainAreas,2)
    figure;
    hold on;
    title(sprintf("%s Spikewidth vs Avg Epoch Mean FR",brainAreas{1,a}))
    ylabel("spike width (ms)")
    xlabel("mean FR (Hz)")
    yline(0.3,'--k')
    if strcmp(brainAreas{1,a},'CA1')
        xline(7,'--k')
    else
        xline(17,'--k')
    end

    for r = 1:size(allNrnStructs,2)

        for tet = 1:size(allNrnStructs(:,r),1)

            if ~isempty(allNrnStructs{tet,r})

                for nrn = 1:size(allNrnStructs{tet,r},2)
                    
                    nrnData = allNrnStructs{tet,r}{1,nrn};

                    if isfield(nrnData,'area') && strcmp(nrnData.area,brainAreas{1,a})
                        
    
                        plot(nrnData.meanrate,nrnData.spikewidth*1000,"ob")
                    end
