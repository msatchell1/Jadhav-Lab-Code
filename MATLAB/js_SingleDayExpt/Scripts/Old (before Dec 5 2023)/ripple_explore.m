...
[C_stateFRs,C_isPyr,C_isInh,FR_allStates,isPyrMat,isInhMat,Occ_allStates,Dur_allStates] = calc_meanrates(brainAreas,C_allstates,C_allspikes);
...
%             FRinh(~isInhMat) = NaN; % Set non-inhibitory cells to NaN
%             FRmeans = mean(FRinh,1,'omitnan');
%             FRstds = std(FRinh,0,1,'omitnan');
%             numnrns = sum(~isnan(FRinh),1);
%             FRsems = FRstds./sqrt(numnrns);
