...
% Try putting it all in one cell array instead. The order of the data
...
% Matrix to hold mean spike rates by epoch, not discriminating by state. 
    ...
    for e = 1:length(C_allspikes{1,r})
        ...
        for nrn = 1:length(nrnsAlltets)
            ...
            if isfield(nrnsAlltets{nrn},'meanrate')
                % For state specific rates, we need to loop through the
                % instances of a specific state and count the number of spikes
                % that fall into that instance. I checked and confirmed
                % that this result matches the .meanrate field for each
                % nrn (only off by 0.0001 for some nrns).
                ratRates(nrn,e) = size(nrnsAlltets{nrn}.data,1)/(nrnsAlltets{nrn}.timerange(2)-nrnsAlltets{nrn}.timerange(1));

            end
        end
    end
    FR_byEpoch = [FR_byEpoch; ratRates]; 
end
...
FR_allStates = cell(size(C_allstates,1),1);
% Main loop to calculate mean firing rate for all states.
for s = 1:size(C_allstates,1)
    ...
    for r = 1:size(C_allstates,2) 
        ...
        FR_allStates{s,1} = [FR_allStates{s,1}; ratMeans];
